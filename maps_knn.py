...
knn_model = joblib.load("rsrp_knn_model.pkl")
...
grid_rsrp = knn_model.predict(grid_scaled)
...
m.save("localized_rsrp_heatmap_knn.html")
print("Heatmap saved as 'localized_rsrp_heatmap_knn.html'")

print(f"Average predicted RSRP: {grid_df['RSRP'].mean():.2f} dBm")
print(f"Good signal (> -100 dBm): {sum(grid_df['RSRP'] >= -100)}/{len(grid_df)}")

grid_df.to_csv("localized_rsrp_predictions_knn.csv", index=False)
print("Predictions exported to 'localized_rsrp_predictions_knn.csv'")
